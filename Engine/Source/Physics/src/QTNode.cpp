...
//  *******************************************************************************************************************
bool cQTNode::DoesCollideWithNode(cRigidBody* const pBody) const
{
...
void cQTNode::Print() const
{
  ...
  str += cStringUtilities::MakeFormatted("ActualRect (left: %f, right: %f, top: %f bottom: %f) ",
                                         m_pActualRect->VGetMinBound().x, m_pActualRect->VGetMaxBound().x,
                                         m_pActualRect->VGetMinBound().y, m_pActualRect->VGetMaxBound().y);

  str += cStringUtilities::MakeFormatted("LooseRect (left: %f, right: %f, top: %f bottom: %f) ",
                                         m_pLooseRect->VGetMinBound().x, m_pLooseRect->VGetMaxBound().x,
                                         m_pLooseRect->VGetMinBound().y, m_pLooseRect->VGetMaxBound().y);
...
cQTNode* const cQTNode::GetChildQuadrant(cRigidBody* const pBody)
{
  ...
  for (unsigned int i = 0; i < m_sSplitSize; i++)
  {
    ...
    {
      float distance =
          pBody->GetCollisionShape()->VGetCenter().DistanceSquared(pChildNode->m_pActualRect->VGetCenter());
...
{
  if (pBody->IsActive() && DoesCollideWithNode(pBody))
  {
    for (auto iter = m_Items.cbegin(); iter != m_Items.cend(); iter++)
    {
      cRigidBody* const pOtherRigidBody = (*iter);
      if (pBody != pOtherRigidBody && pOtherRigidBody->IsInitialized() && pOtherRigidBody->IsActive())
      {
