...
void cHumanView::VOnCreateDevice(IBaseApp* pBaseApp)
{
  ...
  def.vSize = cVector2(static_cast<float>(cGameOptions::GameOptions().iWidth),
                       static_cast<float>(cGameOptions::GameOptions().iHeight));
  if (m_pGame->VGetParamLoader() != NULL)
  {
...
HRESULT cHumanView::OnBeginRender(TICK tickCurrent)
{
  ...
  HRESULT hr = S_OK;
  cGraphics::Instance()->BeginRender();
  return hr;
}
...
void cHumanView::PlaySFX(const cString& strSoundFile)
{
  ...
  {
    shared_ptr<ISoundProcess> pSFXChannelProcess(ISoundProcess::CreateSoundProcess(
        m_hashSFXChannel.GetString(), cGameDirectories::GetSoundSFXDirectory() + strSoundFile,
...
{
  shared_ptr<ISoundProcess> pMusicChannelProcess = ISoundProcess::CreateSoundProcess(
      m_hashMusicChannel.GetString(), cGameDirectories::GetSoundMusicDirectory() + strMusicFile,
...
void cHumanView::FullScreenCheckBoxPressed(const stUIEventCallbackParam& params)
{
  ...
  cGraphics::Instance()->SetFullScreenMode(cGameOptions::GameOptions().bFullScreen);
}
...
void cHumanView::CreateFPSLabel(const cLabelControlDef& def)
{
  ...
  m_pAppWindowControl->VAddChildControl(m_pFpsLabel, weak_ptr<IBaseControl>());
}
