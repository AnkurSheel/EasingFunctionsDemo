...
namespace Base
{
  ...
  inline float DegtoRad(const float x) { return (x * Pi / 180); }
  inline float RadtoDeg(const float x) { return (x * 180 / Pi); }
  ...
  inline int32 RoundReal32ToInt32(const real32 realValue)
  {
    int32 result = static_cast<int32>(realValue + 0.5f);
    return result;
  }
}  // namespace Base
