...
void cXMLFileIO::VParse(const cString& strXML, const unsigned int size)
{
  ...
  SP_ASSERT_ERROR(error == XML_NO_ERROR)
  (m_pDoc->GetErrorStr1())(m_pDoc->GetErrorStr2()).SetCustomMessage("Could not parse XML file");
...
{
  auto curr = m_ElementMap.find(strElementID);
  SP_ASSERT_WARNING(curr != m_ElementMap.end())
  (strAttributeName)(strElementID).SetCustomMessage("Could not find attribute");
  if (curr != m_ElementMap.end())
  {
...
int cXMLFileIO::VGetNodeAttributeAsInt(const cString& strElementID, const cString& strAttributeName)
{
  ...
  SP_ASSERT_WARNING(val.IsValid())
  (strAttributeName)(strElementID).SetCustomMessage("Error in getting attribute as int");
  if (val.IsInvalid())
  {
    return 0;
  }
...
bool cXMLFileIO::VGetNodeAttributeAsBool(const cString& strElementID, const cString& strAttributeName)
{
  ...
  SP_ASSERT_WARNING(val.IsValid())
  (strAttributeName)(strElementID).SetCustomMessage("Error in getting attribute as bool");
  if (val.IsInvalid())
  {
    return false;
  }
