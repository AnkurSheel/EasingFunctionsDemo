...
{
  SP_ASSERT_ERROR(m_AvailableHardDiskSpace > diskSpaceNeeded)
  (diskSpaceNeeded)(m_AvailableHardDiskSpace).SetCustomMessage("Not Enough HardDisk Space");
}

//  *******************************************************************************************************************
void cSystemChecker::VCheckMemory(const uint64 physicalRAMNeeded, const uint64 virtualRAMNeeded)
{
  SP_ASSERT_ERROR(m_AvailablePhysicalMemory > physicalRAMNeeded)
  (physicalRAMNeeded)(m_AvailablePhysicalMemory).SetCustomMessage("Not Enough Physical Memory");
  SP_ASSERT_ERROR(m_AvailableVirtualMemory > virtualRAMNeeded)
  (virtualRAMNeeded)(m_AvailableVirtualMemory).SetCustomMessage("Not Enough Virtual Memory");
...
void cSystemChecker::CalculateHardDiskSpace()
{
  ...
  m_TotalHardDiskSpace =
      static_cast<uint64>(diskfree.total_clusters) * diskfree.sectors_per_cluster * diskfree.bytes_per_sector;
  m_AvailableHardDiskSpace =
      static_cast<uint64>(diskfree.avail_clusters) * diskfree.sectors_per_cluster * diskfree.bytes_per_sector;
}
