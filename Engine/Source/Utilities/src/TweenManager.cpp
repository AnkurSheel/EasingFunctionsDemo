...
float cTweenManager::GetFactor(const cTweenParameters& tween)
{
  ...
  SP_ASSERT_WARNING(findIt != m_EasingFunctions.end())
      .SetCustomMessage("Could not find tweening function for " + EasingTransitions::ToString(tween.m_TransitionType) +
                        " " + EasingType::ToString(tween.m_EasingType));
  ...
  if (findIt != m_EasingFunctions.end())
  {
    ...
    if (pFunction != NULL)
    {
      ...
      {
        SP_ASSERT_WARNING(false)
            .SetCustomMessage("Could not find tweening function type for " + EasingType::ToString(tween.m_EasingType));
      }
    }
  }
  return 0.0f;
