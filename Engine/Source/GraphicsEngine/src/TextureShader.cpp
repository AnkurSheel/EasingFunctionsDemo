...
bool cTextureShader::VCreateLayout(shared_ptr<IResHandle const> shaderHandle)
{
  ...
  HRESULT result = IDXBase::GetInstance()->VGetDevice()->CreateInputLayout(
      pPolygonLayout, iNumberOfLayouts, shaderHandle->GetBuffer(), shaderHandle->GetSize(), &m_pLayout);

  if (FAILED(result))
  {
    SP_ASSERT_ERROR(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Error creating Input Layout");
...
//  *******************************************************************************************************************
bool cTextureShader::VSetShaderParameters(const XMFLOAT4X4& inMatWorld, const XMFLOAT4X4& inMatView,
                                          const XMFLOAT4X4& inMatProjection)
{
  ...
  HRESULT result =
      IDXBase::GetInstance()->VGetDeviceContext()->Map(m_pPixelBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
  if (FAILED(result))
  {
    SP_ASSERT_ERROR(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not lock the pixel Buffer to update with the data");
    return false;
  }

  stPixelBufferData* pPixelData = static_cast<stPixelBufferData*>(mappedResource.pData);
...
bool cTextureShader::CreateSampleState()
{
  ...
  {
    SP_ASSERT_ERROR(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Error creating sampler state");
    return false;
  }
  return true;
}
...
bool cTextureShader::CreatePixelBuffer()
{
  ...
  {
    SP_ASSERT_ERROR(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Error creating pixel buffer");
    return false;
  }
  return true;
}
