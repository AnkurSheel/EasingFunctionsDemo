...
{
	UINT64 hash = cHashedString::CalculateChecksum(strModelFile);
			...
			if (!strLine.IsEmpty())
			{
				...
				if (!strLine.IsEmpty())
				{
					...
					else if (vtokens[0] == "TotalIndexCount")
					{
						...
						pDef->pIndices = DEBUG_NEW UINT64[pDef->iNumberOfIndices];
					}
					// faces/triangles
					else if (vtokens[0] == "t")
					{
...
// *************************************************************************
shared_ptr<stModelDef> cObjModelLoader::Find(const UINT64 ulModelDefHash)
{
