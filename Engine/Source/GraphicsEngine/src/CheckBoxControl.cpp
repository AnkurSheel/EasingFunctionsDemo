...
void cCheckBoxControl::VInitialize(const shared_ptr<IXMLNode const> pXMLNode)
{
	...
	if (pButtonControl != NULL)
	{
		...
		width += m_pTickBox->VGetSize().x;
		height = max(height, m_pTickBox->VGetSize().y);
	}
	...
	// width += pLabel->VGetSize().x;
	// height = max(height, pLabel->VGetSize().y);
	// }
	...
	float fY = VGetSize().y/2.0f;
	if (pLabel != NULL)
	{
		fY += pLabel->VGetSize().y/2.0f;
