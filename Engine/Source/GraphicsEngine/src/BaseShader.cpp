...
bool cBaseShader::VInitialize(const Base::cString & strShaderName)
{
	...
	HRESULT result = IDXBase::GetInstance()->VGetDevice()->CreateBuffer(&matrixBufferDesc, NULL, &m_pMatrixBuffer);
	if (FAILED(result))
	{
...
bool cBaseShader::CreateVertexShader(const Base::cString & strShaderName)
{
	...
	HRESULT result = IDXBase::GetInstance()->VGetDevice()->CreateVertexShader(pData, shaderHandle->GetSize(), NULL, &m_pVertexShader);
...
bool cBaseShader::CreatePixelShader(const Base::cString & strShaderName)
{
	...
	const void * pData = shaderHandle->GetBuffer();
	HRESULT result = IDXBase::GetInstance()->VGetDevice()->CreatePixelShader(pData, shaderHandle->GetSize(), NULL, &m_pPixelShader);
