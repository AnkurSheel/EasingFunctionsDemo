...
//  *******************************************************************************************************************
bool cFontShader::VSetShaderParameters(const XMFLOAT4X4& inMatWorld, const XMFLOAT4X4& inMatView,
                                       const XMFLOAT4X4& inMatProjection)
{
  ...
  if (FAILED(result))
  {
    ...
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not lock the pixel Buffer to update with the matrices data");
    return false;
  }

  PixelBufferType* pPixelData = static_cast<PixelBufferType*>(mappedResource.pData);
