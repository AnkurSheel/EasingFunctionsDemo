...
//  *******************************************************************************************************************
bool cDXBase::VInitialize(const stInitializationParameters& params)
{
  m_bVsyncEnabled = params.vsyncEnabled;
  m_iScreenWidth = params.width;
  m_iScreenHeight = params.height;

  params.bkColor.GetColorComponentsInFloat(m_afBackGroundcolor[0], m_afBackGroundcolor[1], m_afBackGroundcolor[2],
                                           m_afBackGroundcolor[3]);

  if (!SetupRenderTargets(params.width, params.height, params.windowHandle, params.fullScreen))
    return false;
  ...
  SetupViewPort(0, 0, params.width, params.height);

  SetupProjectionMatrix(params.width, params.height, params.screenNear, params.screenDepth);
  XMStoreFloat4x4(&m_matWorld, XMMatrixIdentity());

  XMMATRIX matOrtho = XMMatrixOrthographicLH(params.width, params.height, params.screenNear, params.screenDepth);
  XMStoreFloat4x4(&m_matOrtho, matOrtho);

  VTurnOnAlphaBlending();

  VSetFullScreenMode(params.fullScreen, params.width, params.height);
...
// iCreationFlags |= D3D11_CREATE_DEVICE_DEBUG;
  ...
  for (int i = 0; i < 4; ++i)
  {
    result = D3D11CreateDeviceAndSwapChain(NULL, driverTypes[0], NULL, iCreationFlags, NULL, NULL, D3D11_SDK_VERSION,
                                           &swapChainDesc, &m_pSwapChain, &m_pDevice, &featureLevel, &m_pDeviceContext);
  ...
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not create the swap chain, Direct3D device, and Direct3D device context");
    return false;
  }
...
bool cDXBase::SetupDepthStencilStateFor3D()
{
  ...
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not create the depth stencil state");
    return false;
  }
...
bool cDXBase::SetupDepthStencilStateFor2D()
{
  ...
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not create the depth stencil state for 2D drawing");
    return false;
  }
...
bool cDXBase::CreateDepthStencilView()
{
  ...
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not create the depth stencil view");
    return false;
  }
  return true;
}
...
bool cDXBase::CreateBlendStates()
{
  ...
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Could not create the blend State");
    return false;
  }

  blendStateDescription.RenderTarget[0].BlendEnable = FALSE;

  result = m_pDevice->CreateBlendState(&blendStateDescription, &m_pAlphaDisableBlendingState);
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Could not create the blend State");
...
bool cDXBase::SetupRasterStates()
{
  ...
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Could not create the raster  State");
    return false;
  }
...
bool cDXBase::GetDisplayMode(const int iWidth, const int iHeight)
{
  ...
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Could not create IDXGIFactory factory");
    return false;
  }
  ...
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not create an adapter for the video card");
    return false;
  }
  ...
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result)).SetCustomMessage("Could not Enumerate the monitor");
    return false;
  }
  ...
  if (FAILED(result))
  {
    ...
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage(
            "Could not get the number of modes for the monitor with DXGI_FORMAT_R8G8B8A8_UNORM display format");
    return false;
  }

  DXGI_MODE_DESC* pDisplayModeList = DEBUG_NEW DXGI_MODE_DESC[iNumModes];
  result = pAdapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &iNumModes,
                                              pDisplayModeList);
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not get the display modes for this monitor/video card combination.");
    return false;
  }
...
bool cDXBase::AttachBackBufferToSwapChain()
{
  ...
  bool bSuccess = true;
  HRESULT result = m_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pbackBufferTexture);
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not get a pointer to the back buffer");
    PostQuitMessage(0);
    bSuccess = false;
  }
  else
  {
    ...
    if (FAILED(result))
    {
      SP_ASSERT_FATAL(false)
      (DXGetErrorString(result))(DXGetErrorDescription(result))
          .SetCustomMessage("Could not create a render target view with the back buffer pointer.");
      bSuccess = false;
    }
  }
...
bool cDXBase::CreateDepthStencilBuffer(const int iWidth, const int iHeight)
{
  ...
  if (FAILED(result))
  {
    SP_ASSERT_FATAL(false)
    (DXGetErrorString(result))(DXGetErrorDescription(result))
        .SetCustomMessage("Could not create the 2D texture for the depth buffer");
    return false;
  }
...
//  *******************************************************************************************************************
void cDXBase::SetupProjectionMatrix(const int iWidth, const int iHeight, const float fScreenNear,
                                    const float fScreenDepth)
{
  float fFieldOfView = Pi / 4.0f;
  float fScreenAspect = static_cast<float>(iWidth) / iHeight;
